...
main(int argc, char* argv[])
{
    ...
    std::string simTag = "study2_sched_"+sched+"_duration_" + std::to_string(static_cast<int>(simTime.GetSeconds())) + "_s" + "_ueNum_" + std::to_string(static_cast<int>(ueNumPergNb)) + "_m_run_" + std::to_string(run);
    std::string outputDir = "./scratch/study2/Results";
     ...
     * GrdScenarioHelper documentation to see how the nodes will be distributed.
     */
    ...
    NodeContainer nodes;
    nodes.Create(gNbNum + ueNumPergNb);
	...
	double angle_step = 360.0 / ueNumPergNb;
    ...
    for (uint32_t i = 1; i <= ueNumPergNb; i++) {
        ueContainer.Add(nodes.Get(i));
        i %2 == 0 ? ueLowLatContainer.Add(nodes.Get(i)) : ueVoiceContainer.Add(nodes.Get(i));
    }
