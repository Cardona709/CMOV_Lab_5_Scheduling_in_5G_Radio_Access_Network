...
main(int argc, char* argv[])
{
    ...
    // Where we will store the output files.
    simTag = "study3_sched_"+schedulerType+"_duration_" + std::to_string(static_cast<int>(simTime.GetSeconds())) + "_s" + "_m_run_" + std::to_string(run);
    outputDir = "./scratch/study3/Results";
     ...
     * the gnbs and ue following a pre-defined pattern. 
     */
    int64_t randomStream = 1;

    uint32_t udpPacketSize1=3000;
    uint32_t udpPacketSize2=3000;
    uint32_t lambda1 = 1000;
    uint32_t lambda2 = 1000;

   // Create the nodes for the scenario
    NodeContainer nodes;
    nodes.Create(3);
    ...
    positionAlloc->Add(Vector(100.0, 0.0, 0.0)); // UE1 position
    positionAlloc->Add(Vector(0.0, -100, 0.0)); // UE2 position


    // Set the mobility model for the nodes
    ...
    // Assign the nodes to the corresponding containers
    NodeContainer gNbContainer;
    gNbContainer.Add(nodes.Get(0));


    NodeContainer ueContainer;
    ueContainer.Add(nodes.Get(1));
    ueContainer.Add(nodes.Get(2));

    // Set the UE containers to the corresponding NodeContainer variables
    NodeContainer ueLowLatContainer;
    NodeContainer ueVoiceContainer;
    ueLowLatContainer.Add(ueContainer.Get(0));
    ueVoiceContainer.Add(ueContainer.Get(1));
    ...
    scheduler << "ns3::NrMacScheduler" << subType << schedulerType;  // change this in order to change the scheduler type
    ...
    nrHelper->SetGnbBwpManagerAlgorithmAttribute("GBR_CONV_VOICE",UintegerValue(bwpIdUe2Flow2)); // changed to GBR_CONV_VOICE
    ...
    nrHelper->SetUeBwpManagerAlgorithmAttribute("GBR_CONV_VOICE", UintegerValue(bwpIdUe2Flow2)); //changed to GBR_CONV_VOICE
    ...
    NetDeviceContainer ue1flowNetDev = nrHelper->InstallUeDevice(ueLowLatContainer, allBwps);
    NetDeviceContainer ue2flowsNetDev = nrHelper->InstallUeDevice(ueVoiceContainer, allBwps);
    ...
    for (uint32_t j = 0; j < ueContainer.GetN(); ++j)
    {
        Ptr<Ipv4StaticRouting> ueStaticRouting = ipv4RoutingHelper.GetStaticRouting(
            ueContainer.Get(j)->GetObject<Ipv4>());
    ...
    serverApps.Add(dlPacketSinkUe1flow.Install(ueLowLatContainer));
    serverApps.Add(dlPacketSinkUe2flowsNgbr.Install(ueVoiceContainer));
    serverApps.Add(dlPacketSinkUe2flowsDcGgbr.Install(ueVoiceContainer));
