...
main(int argc, char* argv[])
{
	...
	double distance = 200; //distance between nodes in meters
    ...
    cmd.AddValue("sched", "The scheduler to be used", sched);
    cmd.AddValue("run", "set run", run);

    cmd.Parse(argc, argv);

    // Where we will store the output files.
    std::string simTag = "study1_shed_"+sched+"_duration_" + std::to_string(static_cast<int>(simTime.GetSeconds())) + "_s" + "_distance_" + std::to_string(static_cast<int>(distance)) + "_m_run_" + std::to_string(run);
    std::string outputDir = "./scratch/study1/Results";
    ...
    uint32_t udpPacketSizeULL=3000;
    uint32_t udpPacketSizeBe=3000;
    uint32_t lambdaULL = 1000;
    uint32_t lambdaBe = 1000;


    std::cout << "gNbNum: " << gNbNum << " & UE per gNb: "<< ueNumPergNb << std::endl;
    /*
    ...
    positionAlloc->Add(Vector(0.0, distance, 0.0)); // UE2 position
    ...
    //sched = "RR";  // change this in order to change the scheduler type
